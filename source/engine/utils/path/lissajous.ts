...
export class LissajousCurve {
  ...
  private index: number = 0; // Current index in the track for animation
  ...
  constructor({ a = 3, b = 3, delta = Math.PI / 2, step = 0.01, width, height, uniform = false, scale = 1 }: LissajousCurveParameters) {
    ...
    const dimensions: Vector2 = uniform
      ? new Vector2({ x: Math.min(center.x, center.y) * scale, y: Math.min(center.x, center.y) * scale })
  ...
  /**
   * Updates the position along the curve based on the current index.
   * @returns {Object} An object containing the current x and y coordinates on the curve
   */
  public update(): { x: number, y: number } {
    if (this.index >= this.track.length) this.index = 0;

    const coordinates: { x: number, y: number } = this.track[this.index];
